...
const Checkout = () => {
  ...
  const handleApplyPromo = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/promos/validate?code=${promoCode}`);
      if (!response.ok) throw new Error('Invalid or expired promo code');

      const data = await response.json();
      setDiscount(data.discountValue);
      setPromoMessage(`Promo code applied! ${data.discountValue * 100}% discount`);
    } catch (error) {
  ...
  // Function to SAVE updated card info
  ...
  // Function to SAVE updated address info
  ...
  return (
      <div className="checkout-container-wrapper">
        <div className="checkout-container">
          ...
          {/* Promotions Section */}
          <div className="step">
            <h2 className="step-title">Promotions</h2>
